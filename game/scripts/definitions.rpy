...
init python:
    ...
    DOSSIER_ENTRIES = [
        ...
        ("Argon2", "A modern, memory-hard password-hashing algorithm and the winner of the 2015 Password Hashing Competition. It is deliberately expensive in both CPU and RAM, making GPU-based brute-force attacks extremely slow. Along with bcrypt and scrypt, it is one of the only acceptable algorithms for storing passwords today."),
        ("Bcrypt", "A password-hashing function designed to be computationally expensive and intentionally slow. Its key feature is the 'cost factor' — an integer (e.g. 12) that forces the algorithm to repeat the hashing work 2^N times. At cost 12, each password guess requires 4096 iterations, reducing a modern GPU from millions of guesses per second to roughly 12. Combined with per-user salting, bcrypt defeats both brute-force and rainbow table attacks."),
        ...
        ("Dictionary Attack", "A method of breaking into a password-protected system by systematically entering every word in a dictionary or a list of common passwords (like rockyou.txt). Tools like John the Ripper hash each candidate and compare it to the target. Against fast algorithms like MD5, millions of guesses are tested per second, cracking weak passwords in milliseconds."),
        ...
        ("John the Ripper", "A fast, open-source password cracking tool. Key usage: 'john --wordlist=/usr/share/wordlists/rockyou.txt <hash_file>' for a basic dictionary attack; add '--rules=best64' to apply the 64 most common character-substitution rules; use '--list=rules' to see all available rulesets; specify '--format=bcrypt' when the hash prefix is '$2b$'. It auto-detects many hash types from the prefix."),
        ...
        ("Kali Linux", "A Debian-derived Linux distribution designed for digital forensics and penetration testing. It comes pre-installed with hundreds of security and hacking tools, including John the Ripper and the rockyou.txt wordlist at '/usr/share/wordlists/rockyou.txt'."),
        ...
        ("MD5", "An older hashing algorithm that is now considered cryptographically broken. It produces a 128-bit hash (32 hex characters) with no built-in salting or cost factor. A modern GPU can test billions of MD5 guesses per second, meaning any password found in a dictionary wordlist is cracked in milliseconds. The hash prefix in John the Ripper is 'raw-md5'. Example hash of 'password': 5f4dcc3b5aa765d61d8327deb882cf99."),
        ("Password Salting", "The practice of adding a unique random value (the 'salt') to each password before hashing it. Because every user's salt is different, two users with the same password produce completely different hash strings. Salting defeats precomputed rainbow table attacks and ensures that cracking one hash gives no information about any other. bcrypt, scrypt, and Argon2 all include salting automatically."),
        ("Rainbow Table Attack", "A precomputed lookup table that maps millions of known hash values back to the plaintext passwords that produced them. An attacker who has built such a table can instantly reverse an unsalted hash with a single lookup. Password salting defeats rainbow tables entirely because each salt makes the hash unique, even for identical passwords."),
        ("rockyou.txt", "A wordlist containing approximately 14 million real passwords leaked from the RockYou social-gaming breach of 2009. It is the de-facto standard dictionary for password-cracking exercises and is pre-installed in Kali Linux at '/usr/share/wordlists/rockyou.txt'. Any password appearing in this list is considered compromised and should never be used."),
        ("Rule-Based Mutations (best64)", "A cracking technique where software automatically transforms dictionary words by applying common substitution rules — capitalizing letters, appending numbers, swapping 'e' for '3' or 'o' for '0', etc. John the Ripper's 'best64' ruleset applies 64 such transformations to every dictionary word. A command like 'john --wordlist=rockyou.txt --rules=best64 hash.txt' will test 'monkey', 'M0nk3y', 'M0nk3y!', and thousands of other variations automatically, defeating most 'clever' substitution-based passwords."),
        ("Scrypt", "A memory-hard password-hashing function designed to resist hardware-accelerated attacks. It deliberately requires large amounts of RAM per computation, making it expensive to parallelize on ASICs or GPUs. Along with bcrypt and Argon2, it is one of the only acceptable algorithms for storing passwords today."),
